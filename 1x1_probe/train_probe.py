...
def train_probe(
    ...
    # Calculate the weights for loss function due to high class imbalance
    train_labels = torch.stack([train_ds.dataset[idx][1].int() for idx in train_ds.indices])
    class_counts = torch.bincount(train_labels)
    class_weights = 1.0 / class_counts
    sample_weights = class_weights[train_labels]
    weights = len(train_ds) / (class_counts.shape[0] * class_counts)

    if loss_type == "cross_entropy_weighted":
        ...
        raise ValueError(f"Incorrect `loss_type` specified. Can either be `cross_entropy_weighted`, `focal`, `focal_weighted`, `dice`")
    ...
    for epoch in trange(epochs):
        ...
        for hidden, target in train_loader:
            ...
            logits = model(hidden) # [B,6]
            target_fixed = target.clone().long()
            ce_loss = criterion(logits, target_fixed)
