...
class ProbeVisualizationWrapper(gym.Wrapper):
    ...
    def update_probe_prediction(self, hidden_state):
        ...
        with torch.no_grad():
            ...
            # Expand hidden state for 25 positions
            hidden = hidden_state.repeat(1, self.num_cells, 1)

            # Position encodings
            pos_one_hot = torch.eye(self.num_cells, device=self.device).unsqueeze(0) # Add batch dim

            for trial in range(self.current_trial, self.trials_per_episode):

                trial_one_hot = torch.zeros(self.num_cells, self.trials_per_episode, device=self.device)
                trial_one_hot[:, trial] = 1
                trial_one_hot = trial_one_hot.unsqueeze(0) # Add batch dim

                probe_input = torch.cat(
                    [hidden, pos_one_hot, trial_one_hot],
                    dim=-1
                )

                logits = self.probe_model(probe_input)

                probs = torch.softmax(logits, dim=-1)
                preds = torch.argmax(probs, dim=-1).cpu().numpy()

                grid = preds.reshape(self.grid_size, self.grid_size)
...
class GridActionProbe(nn.Module):
    def __init__(self, hidden_dim=540, num_actions=6):
        super().__init__()
        self.num_actions = num_actions
        self.linear = nn.Linear(hidden_dim, num_actions)

    def forward(self, hidden_states):
        return self.linear(hidden_states) # [B,6]
