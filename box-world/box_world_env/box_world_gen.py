...
def world_gen(n=12, goal_length=3, num_distractor=2, distractor_length=2, seed=None):
    ...
    # Special case: just white key to pick up, zero boxes or distractors
    if goal_length == 1:
        _, _, first_key, agent_pos = sampling_pairs(0, rng, n)
        world[first_key[0], first_key[1]] = np.array(goal_color)
    ...
    distractor_colors = [
        rng.sample(distractor_possible_colors, distractor_length)
    ...
    keys, locks, first_key, agent_pos = sampling_pairs(
        goal_length - 1 + distractor_length * num_distractor, rng, n
    )
    ...
    for i, (distractor_color, root) in enumerate(
        ...
        key_distractor = keys[
            goal_length - 1 + i * distractor_length : goal_length
            - 1
