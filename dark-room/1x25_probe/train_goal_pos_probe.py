...
class ProbeDataset(Dataset):
    def __init__(self, path):
        hidden_states, goal_pos = torch.load(path)
        self.hidden_states = hidden_states.float()
        self.goal_pos = goal_pos.long()
...
class GoalPosProbe(nn.Module):
    ...
    def forward(self, hidden_states):
        return self.linear(hidden_states) # [B,25] - logits for each position
...
def train_probe(
    ...
    if probe_parameters is not None:
        ...
        model = GoalPosProbe(grid_size=room_size).to(device)
    ...
    for epoch in trange(epochs):
        ...
        with torch.no_grad():
            for hidden, target in test_loader:
                hidden = hidden.to(device)
                target = target.to(device).reshape(-1) # [B,25]

                logits = model(hidden) # [B,25]
                # B, _, A = logits.shape
                # logits_flat = logits.reshape((-1, A)) # [Bx25,6]

                preds = logits.argmax(dim=-1).reshape(-1) # [B]
...
def summarize_trial_reports(reports):
    ...
    for row_key in all_rows:
        if row_key not in (f"{idx}" for idx in range(25)):
            continue
